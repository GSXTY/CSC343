...
DROP SCHEMA IF EXISTS LibraryWarmup CASCADE;
CREATE SCHEMA LibraryWarmup;
SET SEARCH_PATH TO LibraryWarmup;

CREATE TYPE LibraryWarmup.holding_type AS ENUM (
	'music', 'books', 'movies', 'audiobooks', 'magazines and newspapers'
);

CREATE TYPE LibraryWarmup.room_type AS ENUM (
	'auditorium', 'meeting', 'theatre'
);

CREATE TYPE LibraryWarmup.week_day AS ENUM (
	'mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'
);

CREATE TYPE LibraryWarmup.age_group AS ENUM (
...
CREATE TABLE IF NOT EXISTS LibraryCatalogue (
	...
	FOREIGN KEY (holding) REFERENCES Holding(id)
		ON DELETE CASCADE ON UPDATE CASCADE,
	CHECK (num_holdings >= copies_available AND copies_available >= 0)
);
